...
class DirectFormatProcessor:
    """直接格式處理器"""
    
    @staticmethod
    def process_pdf_direct_format(pdf_path: str, output_dir: str = "", 
                                answer_pdf_path: str = "") -> Tuple[List[str], Dict[str, Any]]:
        """直接格式PDF處理"""
        
        print(f"\n{'='*70}")
        print(f"📄 {os.path.basename(pdf_path)} (直接格式處理)")
        print(f"{'='*70}")
        ...
        try:
            ...
            # 直接處理特殊格式
            questions = DirectFormatProcessor.extract_questions_direct_format(text)
    ...
    @staticmethod
    def extract_questions_direct_format(text: str) -> List[Dict[str, Any]]:
        """直接格式提取題目"""
        questions = []
        ...
        for pattern in group_patterns:
            ...
            for match in matches:
                ...
                # 直接處理題組中的題目
                group_questions = DirectFormatProcessor.extract_questions_from_group_direct(group_content, start_q, end_q)
                questions.extend(group_questions)
        
        return questions
    
    @staticmethod
    def extract_questions_from_group_direct(group_content: str, start_q: int, end_q: int) -> List[Dict[str, Any]]:
        """直接從題組中提取題目"""
        questions = []
        ...
        for q_num in range(start_q, end_q + 1):
            ...
            for pattern in question_patterns:
                ...
                if match:
                    ...
                    # 直接提取選項
                    options = DirectFormatProcessor.extract_options_direct(question_text)
    ...
    @staticmethod
    def extract_options_direct(question_text: str) -> Dict[str, str]:
        """直接提取選項"""
        options = {}
...
def main():
    ...
    parser = argparse.ArgumentParser(description='直接格式處理器 - 處理特殊格式題目')
    ...
    if os.path.isfile(args.input):
        saved_files, stats = DirectFormatProcessor.process_pdf_direct_format(args.input, args.output, args.answer)
