...
class ManualQuestionGroupProcessor:
    """手動版題組處理器"""
    
    @staticmethod
    def process_pdf_with_manual_groups(pdf_path: str, output_dir: str = "", 
                                     answer_pdf_path: str = "") -> Tuple[List[str], Dict[str, Any]]:
        """使用手動版處理器處理PDF"""
        
        print(f"\n{'='*70}")
        print(f"📄 {os.path.basename(pdf_path)} (手動版題組處理)")
        print(f"{'='*70}")
        ...
        try:
            ...
            # 手動處理題組
            questions = ManualQuestionGroupProcessor.manual_extract_questions(text)
    ...
    @staticmethod
    def manual_extract_questions(text: str) -> List[Dict[str, Any]]:
        """手動提取題目"""
        questions = []
        ...
        for pattern in group_patterns:
            ...
            for match in matches:
                ...
                # 在題組內容中尋找題目
                group_questions = ManualQuestionGroupProcessor.extract_questions_from_group_content(
        ...
        for q_num in range(start_q, end_q + 1):
            ...
            for pattern in question_patterns:
                ...
                if match:
                    ...
                    # 提取選項
                    options = ManualQuestionGroupProcessor.extract_options_from_question(question_text)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='手動版PDF轉CSV工具 - 支援題組處理')
    ...
    if os.path.isfile(args.input):
        saved_files, stats = ManualQuestionGroupProcessor.process_pdf_with_manual_groups(args.input, args.output, args.answer)
