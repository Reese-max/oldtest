...
class ImprovedQuestionGroupProcessor:
    """改進版題組處理器"""
        ...
        for q_num in range(start_q, end_q + 1):
            ...
            for pattern in question_patterns:
                ...
                if match:
                    ...
                    # 提取選項
                    options = ImprovedQuestionGroupProcessor.extract_options_from_question(question_text)
...
def process_pdf_with_improved_groups(pdf_path: str, output_dir: str = "", 
                                   answer_pdf_path: str = "") -> Tuple[List[str], Dict[str, Any]]:
    """使用改進版處理器處理PDF"""
    
    print(f"\n{'='*70}")
    print(f"📄 {os.path.basename(pdf_path)} (改進版題組處理)")
    print(f"{'='*70}")
    ...
    try:
        ...
        # 檢測題組
        groups = ImprovedQuestionGroupProcessor.detect_question_groups(text)
        ...
        if groups:
            ...
            for group in groups:
                ...
                group_questions = ImprovedQuestionGroupProcessor.extract_questions_from_group(group)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='改進版PDF轉CSV工具 - 支援題組處理')
    ...
    if os.path.isfile(args.input):
        saved_files, stats = process_pdf_with_improved_groups(args.input, args.output, args.answer)
