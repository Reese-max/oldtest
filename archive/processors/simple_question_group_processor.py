...
class SimpleQuestionGroupProcessor:
    """簡單版題組處理器"""
    
    @staticmethod
    def process_pdf_with_simple_groups(pdf_path: str, output_dir: str = "", 
                                     answer_pdf_path: str = "") -> Tuple[List[str], Dict[str, Any]]:
        """使用簡單版處理器處理PDF"""
        
        print(f"\n{'='*70}")
        print(f"📄 {os.path.basename(pdf_path)} (簡單版題組處理)")
        print(f"{'='*70}")
        ...
        try:
            ...
            # 檢測題組
            groups = SimpleQuestionGroupProcessor.detect_question_groups(text)
            ...
            if groups:
                ...
                for group in groups:
                    ...
                    group_questions = SimpleQuestionGroupProcessor.extract_questions_from_group(group)
        ...
        for q_num in range(start_q, end_q + 1):
            ...
            for pattern in question_patterns:
                ...
                if match:
                    ...
                    # 提取選項
                    options = SimpleQuestionGroupProcessor.extract_options_from_question(question_text)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='簡單版PDF轉CSV工具 - 支援題組處理')
    ...
    if os.path.isfile(args.input):
        saved_files, stats = SimpleQuestionGroupProcessor.process_pdf_with_simple_groups(args.input, args.output, args.answer)
