...
if group_start != -1:
    ...
    for num in range(51, 55):
        if f'{num} ' in group_text:
            ...
            print(f"\n第{num}題:")
            print(f"原始文本: {repr(question_text)}")
            ...
            print(f"符號列表: {[repr(s) for s in option_symbols]}")
            
            for i, symbol in enumerate(option_symbols):
                print(f"符號 {i}: {repr(symbol)} (長度: {len(symbol)})")
                if symbol in question_part:
                    print(f"  找到符號 {i}")
                    pos = question_part.find(symbol)
                    print(f"    位置: {pos}")
                    print(f"    周圍: {repr(question_part[max(0, pos-5):pos+5])}")
                else:
                    print(f"  未找到符號 {i}")
