...
class UltimateSpecialProcessor:
    """終極特殊格式處理器"""
    
    @staticmethod
    def process_pdf_ultimate_special(pdf_path: str, output_dir: str = "", 
                                   answer_pdf_path: str = "") -> Tuple[List[str], Dict[str, Any]]:
        """終極特殊格式PDF處理"""
        
        print(f"\n{'='*70}")
        print(f"📄 {os.path.basename(pdf_path)} (終極特殊格式處理)")
        print(f"{'='*70}")
        ...
        try:
            ...
            # 直接處理特殊格式
            questions = UltimateSpecialProcessor.extract_questions_ultimate_special(text)
    ...
    @staticmethod
    def extract_questions_ultimate_special(text: str) -> List[Dict[str, Any]]:
        """終極特殊格式提取題目"""
        questions = []
        ...
        for pattern in group_patterns:
            ...
            for match in matches:
                ...
                # 直接處理題組中的題目
                group_questions = UltimateSpecialProcessor.extract_questions_from_group_ultimate_special(group_content, start_q, end_q)
                questions.extend(group_questions)
        
        return questions
    
    @staticmethod
    def extract_questions_from_group_ultimate_special(group_content: str, start_q: int, end_q: int) -> List[Dict[str, Any]]:
        """終極特殊格式從題組中提取題目"""
        questions = []
        ...
        for q_num in range(start_q, end_q + 1):
            ...
            for pattern in question_patterns:
                ...
                if match:
                    ...
                    # 終極特殊格式選項提取
                    options = UltimateSpecialProcessor.extract_options_ultimate_special(question_text)
    ...
    @staticmethod
    def extract_options_ultimate_special(question_text: str) -> Dict[str, str]:
        """終極特殊格式選項提取"""
        options = {}
...
def main():
    ...
    parser = argparse.ArgumentParser(description='終極特殊格式處理器 - 處理特殊格式題目')
    ...
    if os.path.isfile(args.input):
        saved_files, stats = UltimateSpecialProcessor.process_pdf_ultimate_special(args.input, args.output, args.answer)
