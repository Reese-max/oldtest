...
def create_app(config=None):
    ...
    def upload_file():
        ...
        try:
            ...
            return jsonify(
                {"success": True, "task_id": task_id, "filename": filename, "message": f"文件上傳成功: {filename}"}
            )
    ...
    def process_task(task_id):
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                # 記錄處理失敗日誌
                app.logger.error(
                    f"[{g.get('request_id', 'unknown')}] Task failed - " f"TaskID: {task_id}, Error: {task['error']}"
                )
    ...
    def start_crawler():
        ...
        try:
            ...
            if crawler_service.start_task(task_id):
                ...
                return jsonify({"success": True, "task_id": task_id, "message": "爬蟲任務已啟動"})
            else:
                app.logger.error(
                    f"[{g.get('request_id', 'unknown')}] Crawler task start failed - " f"TaskID: {task_id}"
                )
    ...
    def not_found(error):
        """處理 404 未找到錯誤"""
        return jsonify({"error": "資源不存在", "message": "請求的資源未找到", "code": 404}), 404

    @app.errorhandler(405)
    def method_not_allowed(error):
        """處理 405 方法不允許錯誤"""
        return jsonify({"error": "方法不允許", "message": "不支持的 HTTP 方法", "code": 405}), 405

    @app.errorhandler(413)
    def request_entity_too_large(error):
        """處理 413 請求實體過大錯誤"""
        max_mb = app.config.get("MAX_CONTENT_LENGTH", 50 * 1024 * 1024) / (1024 * 1024)
        return jsonify({"error": "文件過大", "message": f"文件大小超過限制（最大 {max_mb:.0f}MB）", "code": 413}), 413
    ...
    def internal_server_error(error):
        ...
        return jsonify({"error": "服務器內部錯誤", "message": "服務器處理請求時發生錯誤，請稍後再試", "code": 500}), 500
    ...
    def handle_exception(error):
        ...
        if hasattr(error, "code"):
            return jsonify({"error": "請求錯誤", "message": str(error), "code": error.code}), error.code

        # 其他異常返回 500
        return jsonify({"error": "服務器錯誤", "message": "處理請求時發生意外錯誤", "code": 500}), 500
