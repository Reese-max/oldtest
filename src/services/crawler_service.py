...
spec = importlib.util.spec_from_file_location("crawler", str(Path(__file__).parent.parent.parent / "考古題下載.py"))
...
class CrawlerService:
    ...
    def _run_crawler(self, task_id: str):
        ...
        try:
            ...
            for idx, year in enumerate(task["years"], 1):
                ...
                # 獲取考試列表
                exams = crawler_module.get_exam_list_by_year(session, year, task["keywords"])
                ...
                for exam in exams:
                    ...
                    # 使用並發或標準下載
                    if use_concurrent:
                        crawler_module.download_exam_concurrent(session, exam, task["save_dir"], task["stats"])
                    else:
                        crawler_module.download_exam(session, exam, task["save_dir"], task["stats"])
    ...
    def _log(self, task_id: str, message: str):
        ...
        with self.lock:
            if task_id in self.tasks:
                self.tasks[task_id]["logs"].append({"timestamp": datetime.now().isoformat(), "message": message})
    ...
    def get_default_keywords(self) -> List[str]:
        ...
        Returns:
            關鍵字列表
        """
        return ["警察人員考試", "一般警察人員考試", "司法人員考試", "國家安全情報人員考試", "移民行政人員考試"]
