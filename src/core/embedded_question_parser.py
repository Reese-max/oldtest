...
class EmbeddedQuestionParser:
    ...
    def _detect_question_groups(self, text: str) -> List[Dict[str, Any]]:
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                groups.append(
                    {"start": start_num, "end": end_num, "marker_pos": match.start(), "marker_end": match.end()}
                )
    ...
    def _extract_passage(self, group_text: str, start_num: int, end_num: int) -> str:
        ...
        for line in lines:
            ...
            is_option_line = len(words) == 4 and all(len(w) > 2 and w[0].islower() for w in words)
    ...
    def _extract_single_question(self, group_text: str, passage: str, question_num: int) -> Dict[str, Any]:
        """提取单个题目"""
    ...
    def validate_questions(self, questions: List[Dict[str, Any]]) -> Tuple[bool, str]:
        ...
        incomplete = []
        for q in questions:
            option_count = sum(1 for opt in ["A", "B", "C", "D"] if q.get(f"选项{opt}", "").strip())
            if option_count < 2:
