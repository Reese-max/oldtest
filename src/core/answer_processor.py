...
class AnswerProcessor:
    ...
    def merge_answers(self, answers: Dict[str, str], corrected_answers: Dict[str, str]) -> Dict[str, str]:
        """
        合併答案和更正答案
        ...
        try:
            ...
            for question_num, corrected_answer in corrected_answers.items():
                if question_num in final_answers:
                    self.logger.debug(
                        f"第{question_num}題答案已更正: {final_answers[question_num]} -> {corrected_answer}"
                    )
