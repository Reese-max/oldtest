        ...
        for match in matches:
            ...
            if 1 <= question_num <= 50:  # 只處理1-50題
                ...
                if len(options) >= 2:  # 降低到2個選項
                    ...
                    self.logger.info(f"✓ 提取測驗題 {question_num}: {question_text[:30]}... (選項: {len(options)})")
            ...
            for question_num in range(start_num, end_num + 1):
                ...
                if f'{question_num} ' in group_text:
                    ...
                    if not any(c in question_part for c in EMBEDDED_SYMBOLS):
                        ...
                        questions.append({
                            'question_number': str(question_num),
                            'question_text': f"第{question_num}題填空",
                        ...
                        if len(options) >= 2:
                            ...
                            self.logger.info(f"✓ 提取題組題 {question_num}: {question_text[:30]}... (選項: {len(options)})")
