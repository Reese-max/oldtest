        ...
        for match in matches:
            ...
            if 1 <= question_num <= 50:  # 只處理1-50題
                ...
                if len(options) >= 2:  # 降低到2個選項
                    ...
                    self.logger.info(f"✓ 提取測驗題 {question_num}: {question_text[:30]}... (選項: {len([o for o in options if o])})")
                ...
                if f'{question_num} ' in group_text:
                    ...
                    if not any(c in question_part for c in EMBEDDED_SYMBOLS):
                        ...
                        if len(options) >= 2:
                            ...
                            self.logger.info(f"✓ 提取題組題 {question_num}: {question_text[:30]}... (選項: {len([o for o in options if o])})")
