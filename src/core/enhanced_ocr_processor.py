...
# OCR 參數常量
OCR_MIN_DPI_SCANNED = 300  # 掃描版最小DPI
OCR_MIN_ZOOM_SCANNED = 2.0  # 掃描版最小縮放
OCR_DPI_TEXT_BASED = 150  # 文字型PDF的DPI
OCR_ZOOM_TEXT_BASED = 1.5  # 文字型PDF的縮放
OCR_DPI_HYBRID = 250  # 混合型PDF的DPI
OCR_ZOOM_HYBRID = 1.8  # 混合型PDF的縮放

# OCR 檢測和閾值常量
OCR_DET_DB_THRESH_SCANNED = 0.2  # 掃描版檢測閾值
OCR_CONFIDENCE_THRESH_SCANNED = 0.6  # 掃描版信心閾值
OCR_DET_DB_THRESH_HYBRID = 0.3  # 混合型檢測閾值
...
class EnhancedOCRProcessor:
        ...
        try:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                pdf_type = self._classify_pdf_type(
                    text_page_ratio, scanned_page_ratio, avg_chars_per_page, total_images
                )
    ...
    def _classify_pdf_type(self, text_ratio: float, scanned_ratio: float, avg_chars: float, total_images: int) -> str:
        """
        分類 PDF 類型
    ...
    def optimize_ocr_parameters(
        self, pdf_path: str, pdf_type: str, detection_details: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if pdf_type == PDFType.SCANNED:
            # 掃描版：提高 DPI 和閾值精度
            params["dpi"] = max(params["dpi"], OCR_MIN_DPI_SCANNED)
            params["zoom"] = max(params["zoom"], OCR_MIN_ZOOM_SCANNED)
            params["det_db_thresh"] = OCR_DET_DB_THRESH_SCANNED  # 更敏感的檢測
            params["confidence_threshold"] = OCR_CONFIDENCE_THRESH_SCANNED  # 較高的信心閾值
            self.logger.info("  📊 掃描版優化: 高DPI + 敏感檢測")

        elif pdf_type == PDFType.TEXT_BASED:
            # 文字型：降低處理需求
            params["dpi"] = OCR_DPI_TEXT_BASED
            params["zoom"] = OCR_ZOOM_TEXT_BASED
            params["skip_ocr"] = True  # 標記可跳過 OCR
            self.logger.info("  📝 文字型優化: 低DPI（可跳過OCR）")

        elif pdf_type == PDFType.HYBRID:
            # 混合型：平衡策略
            params["dpi"] = OCR_DPI_HYBRID
            params["zoom"] = OCR_ZOOM_HYBRID
            params["det_db_thresh"] = OCR_DET_DB_THRESH_HYBRID
    ...
    def validate_ocr_quality(self, text: str, pdf_path: str = None) -> Tuple[str, Dict[str, Any]]:
        ...
        # 5. 綜合評分
        overall_score = (
            metrics["confidence_score"] * 0.4 + metrics["completeness_score"] * 0.3 + metrics["readability_score"] * 0.3
        )
    ...
    def _ocr_single_page(self, pdf_path: str, page_num: int) -> str:
        ...
        if self._ocr_processor is None:
            ...
            self._ocr_processor = OCRProcessor(use_gpu=self.use_gpu, lang=self.lang)
    ...
    def _extract_with_optimized_ocr(self, pdf_path: str, params: Dict[str, Any]) -> str:
        ...
        if self._ocr_processor is None:
            ...
            self._ocr_processor = OCRProcessor(
                use_gpu=params.get("use_gpu", self.use_gpu), lang=params.get("lang", self.lang)
            )

        # 使用 OCR 提取
        text = self._ocr_processor.extract_text_from_pdf(
            pdf_path, use_structure=False, confidence_threshold=params.get("confidence_threshold", 0.5)
        )
