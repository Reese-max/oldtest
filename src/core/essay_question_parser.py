...
class EssayQuestionParser:
    ...
    def _detect_question_type(self, text: str) -> QuestionType:
        ...
        choice_count = sum(1 for pattern in choice_patterns if re.search(pattern, text))
    ...
    def validate_coverage(self, questions: List[Dict[str, Any]], expected_count: int = None) -> Tuple[bool, str]:
        """
        驗證題目覆蓋率
        ...
        question_nums = sorted([q[CSV_COLUMN_QUESTION_NUM] for q in questions])
