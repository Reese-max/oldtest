...
class EssayDetector:
    ...
    def detect_essay_exam(self, text: str) -> Dict[str, any]:
        ...
        if not text or len(text) < 50:
            return {"is_essay": False, "confidence": 0.0, "features": {}, "reason": "文本內容過短，無法判斷"}

        features = self._extract_features(text)
        is_essay, confidence, reason = self._make_decision(features)

        return {"is_essay": is_essay, "confidence": confidence, "features": features, "reason": reason}
    ...
    def _make_decision(self, features: Dict) -> tuple:
        ...
        if features["chinese_numbers"]["has_chinese_numbers"]:
            ...
            score += weight
            reasons.append(f"發現 {features['chinese_numbers']['count']} 個中文數字題號")

        # 反向特徵（支持選擇題）
        if features["choice_marks"]["has_choice_marks"]:
            penalty = min(features["choice_marks"]["count"] / 20, 1.0) * 0.3
            score -= penalty
            reasons.append(f"發現 {features['choice_marks']['count']} 個選擇題標記（減分）")
        ...
        if features["arabic_numbers"]["has_arabic_numbers"]:
            ...
            if features["arabic_numbers"]["count"] > 10:
                penalty = 0.25
                score -= penalty
                reasons.append(f"發現 {features['arabic_numbers']['count']} 個阿拉伯數字題號（減分）")

        # 特殊情況：幾乎沒有任何特徵
        if (
            not features["essay_keywords"]["has_keywords"]
            and not features["score_marks"]["has_scores"]
            and not features["chinese_numbers"]["has_chinese_numbers"]
            and not features["arabic_numbers"]["has_arabic_numbers"]
        ):
