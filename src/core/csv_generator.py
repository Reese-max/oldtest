...
class CSVGenerator:
    ...
    def generate_questions_csv(self, questions: List[Dict[str, Any]], answers: Dict[str, str], output_path: str) -> str:
        """
        生成題目CSV檔案
        ...
        try:
            ...
            if not questions:
                ...
                csv_data = [
                    self._build_question_row(question, answers, include_corrected=False) for question in questions
                ]
    ...
    def generate_question_groups_csv(
        self, questions: List[Dict[str, Any]], answers: Dict[str, str], output_dir: str
    ) -> List[str]:
        """
        生成題組分類CSV檔案
    ...
    def _save_csv_data(self, csv_data: List[Dict[str, Any]], output_path: str) -> None:
        ...
        try:
            ...
            # 保存CSV
            df.to_csv(output_path, index=False, encoding=self.config.output_encoding, sep=self.config.csv_delimiter)
        ...
        if questions:
            for i, q in enumerate(questions):
                if not isinstance(q, dict):
                    raise CSVGenerationError(f"questions[{i}] 必須是字典，收到類型: {type(q).__name__}")
    ...
    def _calculate_difficulty(self, question: Dict[str, Any]) -> str:
        ...
        from ..utils.constants import (
            ...
            DIFFICULTY_HARD_LENGTH,
            DIFFICULTY_MEDIUM_LENGTH,
        )
