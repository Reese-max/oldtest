...
from ..utils.constants import (
    DEFAULT_OUTPUT_DIR,
    FILE_PATTERN_ANSWER,
    FILE_PATTERN_CORRECTED_ANSWER,
    FILE_PATTERN_GOOGLE_CSV,
    FORMAT_TYPE_COMPREHENSIVE,
    FORMAT_TYPE_EMBEDDED,
    FORMAT_TYPE_ESSAY,
    FORMAT_TYPE_MIXED,
    FORMAT_TYPE_STANDARD,
    KEYWORDS_ENGLISH_ESSAY,
    KEYWORDS_ESSAY_PART,
    KEYWORDS_ESSAY_SECTION,
    KEYWORDS_QUESTION_GROUP_END,
    KEYWORDS_QUESTION_GROUP_START,
    KEYWORDS_TEST_PART,
    KEYWORDS_TEST_SECTION,
    UNICODE_OPTION_SYMBOLS,
)
...
class ArchaeologyProcessor:
    ...
    def process_pdf(
        ...
        try:
            ...
            # 4. 提取並合併答案
            answers, corrected_answers = self._extract_and_merge_answers(answer_pdf_path, corrected_answer_pdf_path)
            ...
            report_path = os.path.join(
                output_dir, f"{os.path.splitext(os.path.basename(pdf_path))[0]}_scan_report.json"
            )
    ...
    def _extract_and_merge_answers(
        self, answer_pdf_path: Optional[str], corrected_answer_pdf_path: Optional[str]
        ...
        # 提取更正答案
        corrected_answers = self._extract_corrected_answers_from_pdf(corrected_answer_pdf_path)
    ...
    def _build_result(self, result_data: ProcessingResult) -> Dict[str, Any]:
        ...
        Returns:
            處理結果字典
        """
        stats = self._generate_statistics(result_data.questions, result_data.answers, result_data.corrected_answers)
    ...
    def _smart_parse_questions(self, text: str, pdf_path: str) -> List[Dict[str, Any]]:
        ...
        if self.scan_tracker and questions:
            ...
            for q in questions:
                ...
                if question_num:
                    self.scan_tracker.register_question(question_num, parser_name, question_text)
    ...
    def _parse_standard(self, text: str) -> List[Dict[str, Any]]:
        ...
        if len(questions) < 2:
            if essay_result["is_essay"]:  # 中低信心度的申論題
                ...
                self.logger.warning(f"⚠️  未識別為申論題，可能是特殊格式或掃描品質不佳\n" f"   {essay_result['reason']}")
    ...
    def _is_comprehensive_format(self, text: str) -> bool:
        ...
        has_question_groups = KEYWORDS_QUESTION_GROUP_START in text and KEYWORDS_QUESTION_GROUP_END in text
    ...
    def process_directory(self, input_dir: str, output_dir: str = DEFAULT_OUTPUT_DIR) -> Dict[str, Any]:
        """
        處理目錄中的所有PDF檔案
        ...
        try:
            ...
            self.logger.success(
                f"目錄處理完成: {successful_count}/{len(pdf_files)} 個檔案成功，共 {total_questions} 題"
            )
            return summary
    ...
    def _generate_statistics(
        self, questions: List[Dict[str, Any]], answers: Dict[str, str], corrected_answers: Dict[str, str]
    ) -> Dict[str, Any]:
        """生成統計資訊"""
        try:
            # 題目統計
