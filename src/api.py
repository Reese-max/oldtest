...
from .utils.constants import FILE_PATTERN_GOOGLE_CSV, FILE_PATTERN_GOOGLE_SCRIPT
...
class ArchaeologyAPI:
    ...
    def process_single_pdf(
        ...
        try:
            ...
            # 處理PDF
            result = self.processor.process_pdf(pdf_path, answer_pdf_path, corrected_answer_pdf_path, output_dir)
    ...
    def process_directory(
        self, input_dir: str, output_dir: str = "output", generate_script: bool = True
    ) -> Dict[str, Any]:
        """
        處理目錄中的所有PDF檔案
    ...
    def generate_script_from_csv(self, csv_path: str, output_path: Optional[str] = None) -> str:
        """
    ...
    def _generate_google_script(self, result: Dict[str, Any], pdf_path: str, output_dir: str) -> None:
        """
...
def main():
    ...
    try:
        if os.path.isfile(args.input):
            # 處理單一檔案
            result = api.process_single_pdf(args.input, args.answer, args.corrected, args.output, not args.no_script)
        else:
            # 處理目錄
            result = api.process_directory(args.input, args.output, not args.no_script)
