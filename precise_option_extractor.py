...
class PreciseOptionExtractor:
    """精確選項提取器"""
    
    @staticmethod
    def process_pdf_precise(pdf_path: str, output_dir: str = "", 
                          answer_pdf_path: str = "") -> Tuple[List[str], Dict[str, Any]]:
        """精確PDF處理"""
        
        print(f"\n{'='*70}")
        print(f"📄 {os.path.basename(pdf_path)} (精確選項提取)")
        print(f"{'='*70}")
        ...
        try:
            ...
            # 檢測題組
            groups = PreciseOptionExtractor.detect_question_groups(text)
            ...
            if groups:
                ...
                for group in groups:
                    ...
                    group_questions = PreciseOptionExtractor.extract_questions_from_group_precise(group)
                    questions.extend(group_questions)
            
            # 處理非題組的單獨題目
            individual_questions = PreciseOptionExtractor.extract_individual_questions_precise(text)
    ...
    @staticmethod
    def extract_questions_from_group_precise(group: Dict[str, Any]) -> List[Dict[str, Any]]:
        """精確從題組中提取題目"""
        questions = []
        ...
        for q_num in range(start_q, end_q + 1):
            ...
            for pattern in question_patterns:
                ...
                if match:
                    ...
                    # 精確提取選項
                    options = PreciseOptionExtractor.extract_options_precise(question_text)
    ...
    @staticmethod
    def extract_options_precise(question_text: str) -> Dict[str, str]:
        """精確選項提取"""
        options = {}
    ...
    @staticmethod
    def extract_individual_questions_precise(text: str) -> List[Dict[str, Any]]:
        """精確提取單獨題目"""
        questions = []
...
def main():
    ...
    parser = argparse.ArgumentParser(description='精確選項提取器 - 修復選項提取問題')
    ...
    if os.path.isfile(args.input):
        saved_files, stats = PreciseOptionExtractor.process_pdf_precise(args.input, args.output, args.answer)
