...
class SpecialFormatProcessor:
    """特殊格式處理器"""
    
    @staticmethod
    def process_pdf_special_format(pdf_path: str, output_dir: str = "", 
                                 answer_pdf_path: str = "") -> Tuple[List[str], Dict[str, Any]]:
        """特殊格式PDF處理"""
        
        print(f"\n{'='*70}")
        print(f"📄 {os.path.basename(pdf_path)} (特殊格式處理)")
        print(f"{'='*70}")
        ...
        try:
            ...
            # 直接處理特殊格式
            questions = SpecialFormatProcessor.extract_questions_special_format(text)
    ...
    @staticmethod
    def extract_questions_special_format(text: str) -> List[Dict[str, Any]]:
        """特殊格式提取題目"""
        questions = []
        ...
        for pattern in group_patterns:
            ...
            for match in matches:
                ...
                # 直接處理題組中的題目
                group_questions = SpecialFormatProcessor.extract_questions_from_group_special(group_content, start_q, end_q)
                questions.extend(group_questions)
        
        return questions
    
    @staticmethod
    def extract_questions_from_group_special(group_content: str, start_q: int, end_q: int) -> List[Dict[str, Any]]:
        """特殊格式從題組中提取題目"""
        questions = []
        ...
        for q_num in range(start_q, end_q + 1):
            ...
            for pattern in question_patterns:
                ...
                if match:
                    ...
                    # 特殊格式選項提取
                    options = SpecialFormatProcessor.extract_options_special(question_text)
    ...
    @staticmethod
    def extract_options_special(question_text: str) -> Dict[str, str]:
        """特殊格式選項提取"""
        options = {}
...
def main():
    ...
    parser = argparse.ArgumentParser(description='特殊格式處理器 - 處理特殊格式題目')
    ...
    if os.path.isfile(args.input):
        saved_files, stats = SpecialFormatProcessor.process_pdf_special_format(args.input, args.output, args.answer)
