...
class TestArchaeologyAPI(unittest.TestCase):
    ...
    def test_process_single_pdf_success(
        ...
        mock_parse_questions,
        mock_extract_method,
    ):
        """測試成功處理單一PDF"""
        # 模擬返回值
        mock_extract_method.return_value = {"text": "測試PDF內容", "method": "pdfplumber", "score": 0.95}
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = self.api.process_single_pdf("test.pdf", output_dir=temp_dir, generate_script=False)
    ...
    def test_process_directory_success(self, mock_process_pdf):
        """測試成功處理目錄"""
        # 模擬單一檔案處理結果
        mock_process_pdf.return_value = {"success": True, "questions_count": 5, "csv_files": ["/tmp/file1.csv"]}
