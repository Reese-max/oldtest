...
class TestArchaeologyAPIExtended(unittest.TestCase):
    ...
    def test_process_single_pdf_with_script_generation(
        self, mock_group_csv, mock_google_csv, mock_general_csv, mock_script_gen, mock_parse, mock_extract
    ):
        """測試處理單一PDF並生成Script"""
        # 模擬返回值
        mock_extract.return_value = {"text": "測試內容", "method": "pdfplumber", "score": 0.95}
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = self.api.process_single_pdf("test.pdf", output_dir=temp_dir, generate_script=True)
    ...
    def test_process_directory_with_script(self, mock_process_dir):
        ...
        mock_process_dir.return_value = {
            ...
            "total_questions": 10,
            "results": [
                {"success": True, "csv_files": ["output/file1.csv", "output/file1_Google表單.csv"]},
                {"success": True, "csv_files": ["output/file2.csv", "output/file2_Google表單.csv"]},
            ],
        }
