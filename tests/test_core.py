...
class TestCSVGenerator(unittest.TestCase):
    ...
    def test_generate_questions_csv(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result_path = self.generator.generate_questions_csv(self.test_questions, self.test_answers, output_path)
    ...
    def test_calculate_difficulty(self):
        ...
        medium_question = {
            "題目": "這是一個中等長度的題目，包含足夠的內容來測試難度計算功能，確保能夠正確判斷為中等難度，這個題目應該超過五十個字元但小於一百個字元"
        }
        # 困難難度需要超過100字元
        hard_question = {
            "題目": "這是一個非常長的題目，包含大量的內容和詳細的描述，用來測試困難級別的題目難度計算功能，確保系統能夠正確識別和分類不同難度的題目，這個題目應該被判斷為困難級別，因為它超過了一百個字元，這是一個很長的題目，需要更多內容"
        }
