...
class FinalImprovedProcessor:
    """最終改進版處理器"""
    
    @staticmethod
    def process_pdf_final_improved(pdf_path: str, output_dir: str = "", 
                                 answer_pdf_path: str = "") -> Tuple[List[str], Dict[str, Any]]:
        """最終改進版PDF處理"""
        
        print(f"\n{'='*70}")
        print(f"📄 {os.path.basename(pdf_path)} (最終改進版處理)")
        print(f"{'='*70}")
        ...
        try:
            ...
            # 直接處理特殊格式
            questions = FinalImprovedProcessor.extract_questions_final_improved(text)
    ...
    @staticmethod
    def extract_questions_final_improved(text: str) -> List[Dict[str, Any]]:
        """最終改進版提取題目"""
        questions = []
        ...
        for pattern in group_patterns:
            ...
            for match in matches:
                ...
                # 直接處理題組中的題目
                group_questions = FinalImprovedProcessor.extract_questions_from_group_final(group_content, start_q, end_q)
                questions.extend(group_questions)
        
        return questions
    
    @staticmethod
    def extract_questions_from_group_final(group_content: str, start_q: int, end_q: int) -> List[Dict[str, Any]]:
        """最終版從題組中提取題目"""
        questions = []
        ...
        for q_num in range(start_q, end_q + 1):
            ...
            for pattern in question_patterns:
                ...
                if match:
                    ...
                    # 直接提取選項
                    options = FinalImprovedProcessor.extract_options_final(question_text)
    ...
    @staticmethod
    def extract_options_final(question_text: str) -> Dict[str, str]:
        """最終版選項提取"""
        options = {}
...
def main():
    ...
    parser = argparse.ArgumentParser(description='最終改進版處理器 - 處理特殊格式題目')
    ...
    if os.path.isfile(args.input):
        saved_files, stats = FinalImprovedProcessor.process_pdf_final_improved(args.input, args.output, args.answer)
