...
"""
    ...
    def generate_script_from_csv(self, csv_path: str, output_path: str = None) -> str:
        ...
        # 生成腳本
        script_content = self.script_template.replace("{exam_title}", "考古題練習")
        script_content = script_content.replace("{total_questions}", str(len(df)))
        script_content = script_content.replace("{questions_data}", questions_data)
        script_content = script_content.replace("{correct_answers}", correct_answers)
