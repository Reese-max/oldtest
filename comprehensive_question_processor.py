...
class ComprehensiveQuestionProcessor:
    """綜合題目處理器"""
    ...
    def process_all_subjects(base_dir: str, output_dir: str = "") -> Dict[str, Any]:
        ...
        for subject in subjects:
            ...
            if os.path.isdir(subject_path):
                ...
                # 處理科目
                subject_result = ComprehensiveQuestionProcessor.process_subject(subject_path, subject, output_dir)
                if subject_result:
    ...
    def process_subject(subject_path: str, subject_name: str, output_dir: str) -> Dict[str, Any]:
        ...
        # 處理PDF
        questions = ComprehensiveQuestionProcessor.extract_questions_from_pdf(question_file, answer_file)
    ...
    def extract_questions_from_pdf(pdf_path: str, answer_pdf_path: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 檢測題組
            groups = ComprehensiveQuestionProcessor.detect_question_groups(text)
            ...
            if groups:
                # 處理題組
                for group in groups:
                    group_questions = ComprehensiveQuestionProcessor.extract_questions_from_group(group)
                    questions.extend(group_questions)
            
            # 處理非題組的單獨題目
            individual_questions = ComprehensiveQuestionProcessor.extract_individual_questions(text)
        ...
        for q_num in range(start_q, end_q + 1):
            ...
            for pattern in question_patterns:
                ...
                if match:
                    ...
                    # 提取選項
                    options = ComprehensiveQuestionProcessor.extract_options_from_question(question_text)
    ...
    @staticmethod
    def extract_individual_questions(text: str) -> List[Dict[str, Any]]:
        """提取單獨題目"""
        questions = []
...
def main():
    ...
    parser = argparse.ArgumentParser(description='綜合題目處理器 - 支援所有題型')
    ...
    if os.path.isdir(args.input):
        results = ComprehensiveQuestionProcessor.process_all_subjects(args.input, args.output)
